...
- No gym time-zone field was present in these observed responses. Inspection of the gym page did not establish an authoritative time zone. Automatic time-zone discovery remains unresolved. Issue #3 originally gated date queries on explicit user-confirmed zone configuration; the [2026-09-24 default-zone decision](adr/2026-09-24-default-gym-time-zone.md) later allowed an explicitly marked `Europe/Madrid` assumption. The historical issue #3 observations below retain their original validation context.
...
Workout feed entries contain `wodClass`, `day`, `ejerRate` and `TIPOWODs`. The official renderer labels `wodClass` as the training class. Detail `recordDate` is displayed as the workout date and differs from `publishDate`: the available WOD observed on 22 September had intended date 22 September 2026 and publication date 21 September 2026. The full Spanish date establishes the year without borrowing publication time. The workout had three blocks and nine exercise entries. Source block titles come from the feed; notes and exercise prescriptions come from the detail. Source encodings are retained without invented unit meanings. Difficulty variants were outside the original projection; the 2026-09-23 investigation below verified and added them.
...
## Published workout difficulty variants (2026-09-23)

The user supplied three screenshots of the same next-day WOD with `RX`, `INTERMEDIO` and `SCALED` selected, plus its gym `/api/activity?timeLineFormat=0&timeLineContent=7` URL. An anonymous request to that URL returned HTML rather than the activity JSON. Authenticated read-only inspection first verified the configured account's membership in the named gym, then read the supplied feed view and the WOD detail for 24 September 2026. The feed contained 32 entries; both WOD and Metcon publications exposed scaling labels. No full response, cookie or personal identifier was retained.

The official gym renderer collects distinct labels from `TIPOWODs[].scaledops` in source order. For each label it selects the same index in a block's `scaledver`, falling back to the base block when that label has no block replacement. For exercises belonging to a labeled block, it selects the same index in each exercise's `scaledver`; exercises in unlabeled blocks keep their base form. The observed WOD detail had three blocks and ten exercises. Its final block carried `['SCALED', 'INTERMEDIO', 'RX']` with three block replacements, and its associated exercises each carried three replacements. The earlier shared blocks had no labels. This explains the screenshots' common warm-up and level-specific final AMRAP.

An independent official MCP SDK stdio comparison subsequently checked the 24 September WOD and Metcon against fresh authenticated feed/detail responses. Each class had one matching future publication and three returned variants; labels, block notes, exercise names and all projected prescription values matched the selected source versions. This verifies the relationship for the observed gym and date, not a universal three-level rule. Other labels, incomplete source variants, and other gyms remain unverified; malformed relationships are reported as unsupported instead of guessed. See [validation](validation.md) and [the amended applicability decision](adr/2026-09-22-published-workout-applicability.md).

## Personal activity recorded results (2026-09-23)

The user supplied a response from `GET /api/activity/workout?SEID=<activity-id>` and requested inclusion of recorded block results. This is user-provided evidence, not a new live HTTP verification. The sample's `TIPOWODs` contains `res`, `reps`, `time`, `rondas`, `rx` and `rxstr`, alongside the already projected notes and prescription fields. The user explicitly confirmed that `time` is measured in seconds. No conversion or unit inference is applied to other numeric fields. Null result values and empty source labels occur; `rx=false` also appears on a free-text block with no score, so it cannot independently establish a scaled workout. Exercise `tWOD*` fields repeat block information and are not projected as independent results.

Personal activity now exposes allowlisted `blocks[].result` fields. Missing values remain absent, nulls and zeroes are preserved, and deleted blocks expose an empty result. Invalid allowed-field types make detail coverage incomplete under the existing recovery policy. Incidental `PRs`, `PRsGlobal` and profiles remain discarded; the subsequently added current-result description exception for `chartData` is described below. Non-null `res`/`rondas` variants are fixture-tested numeric support, not evidence from this sample. The cache-busting `_` parameter in the supplied URL is not required by the established client contract. See [the activity decision](adr/2026-09-22-personal-activity-calendar-coverage.md).

### Current-result description

The same supplied sample has `chartData` keys matching `TIPOWODs[].id` (not `pwid`), with a row whose `idAction` matches the requested activity ID. Its `desc` is `7R`. The user confirmed that this means seven completed rounds of the complexes in that class, while explicitly leaving other gyms' conventions unverified. Preserve this description verbatim as `blocks[].result.desc`; do not parse a universal rounds count or substitute it for `rondas` or `reps`.

Only rows linked by both block ID and current activity ID contribute a description. Historical rows and unrelated block keys are ignored; no chart history or auxiliary fields are exposed. Missing matches leave `desc` absent; null and empty labels are preserved. Conflicting matching labels or malformed matching descriptions make coverage incomplete instead of choosing an arbitrary result. This join is supported by the supplied sample, not a newly performed live query or multi-gym verification.

## Exercise value and load units (2026-09-24)

The user reported that MCP answers lost the units of lifts and gave examples from the gym UI. After verifying the configured account's membership, authenticated read-only reads of the selected gym's publication feed and workout details compared the observed 23 and 24 September 2026 WODs, plus the 23 September GAP and Mobility classes. Only selected public exercise fields were inspected; no full response, credentials or account identifiers were retained.

- The 24 September WOD's alternating dumbbell snatch uses `formaReg=4`, `tipoud=0`, and level-specific `valor2` values `15/10`, `17.5/12.5` and `22.5/15`. The official renderer maps `tipoud=0` to `kg` for this format.
- The 23 September WOD's Front Squat uses `formaReg=4`, `valor1=["3","3","3"]`, `valor2="85/85"` and `tipoud=4`. The renderer maps code 4 to `%RM`, establishing a relative load rather than kilograms. The user's interpretation is 85% of their personal maximum; the server preserves `%RM` without calculating a maximum or absolute weight.
- The same WOD's `Descanso Rest` entries use `formaReg=1` with `valor1=["30","30","30"]` or `["90","90","90"]`; the 23 September GAP includes `["60","60","60"]`. The renderer treats these as seconds and displays a divisible-by-60 value such as 60 seconds as `1'`. An empty `valor1=[""]` supplies no duration.
- The 23 September WOD's Dual Kettlebell Front Rack Lunge has `formaReg=4`, `valor1=["10","10","10"]`, `valor2=null` and `tipoud=1`. It has repetitions but no load value, so the otherwise recognized `lbs` code must not become a displayed load.
- The 23 September Mobility Dual KB Front Rack Carry has `formaReg=6`, `valor1=["30","30","30"]`, `tipoud=0`, `tipoud2=0` and `valor2=null`. The renderer uses `tipoud=0` as `m` for distance; with no load value, `tipoud2=0` does not imply a displayed kilogram load.

The inspected official renderer defines `udsPeso = ["kg", "lbs", "pood", "%BW", "%RM", "RIR", "RPE"]` and `udsDistancia = ["m", "mi", "yd", "ft", "steps", "km"]`. Its `formaReg` branches use `valor1` as time (`1`), distance (`2`), repetitions (`3`), repetitions plus load (`4`), calories (`5`), or distance plus load (`6`). It selects the load unit from `tipoud` for `4` and `tipoud2` for `6`. These are observed renderer contracts for the tested gym, not a guarantee for every AimHarder variant. See [the amended decision](adr/2026-09-22-published-workout-applicability.md) and [validation](validation.md).
