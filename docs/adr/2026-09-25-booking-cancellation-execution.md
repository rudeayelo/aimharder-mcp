...
Send at most one form `POST /api/cancelBook` with the verified schedule reservation `id` and `late=0`. Do not use this POST as a preflight. The public official frontend shows this route, fields, and `cancelState` branches, but authenticated request and response semantics remain unverified. Never replay a write after a timeout, connection loss, or authentication interruption. Reconcile through fresh daily schedule and upcoming-booking reads, with read-only reauthentication if needed. Only `cancelState=1` plus a nonconflicting fresh cancelled schedule retaining the same reservation identifier supports `confirmed`; a cancelled row for a different or unidentifiable reservation remains uncertain. `cancelState=3` plus a still booked schedule supports `rejected`. `cancelState=2` plus a still booked schedule is `pending-credit-loss` and sends no second write. Unsupported responses, incomplete reads, and conflicting views are `uncertain`. A fresh read does not establish a credit refund or balance.
