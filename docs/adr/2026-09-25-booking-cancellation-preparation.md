...
The preview shows the gym, class, local times, current booked state, possible credit loss, and unknown balance/entitlement period. At 9NBC, warn at or inside its published 90-minute credit-loss boundary using gym-local wall time; do not extend this rule to other gyms or turn it into a claim about exact API enforcement. A 256-bit in-memory reference expires after two minutes, binds the account, gym, reservation and preview, and is single-use. The initial cancellation POST is never used as a preflight because the official frontend indicates it may itself cancel.
