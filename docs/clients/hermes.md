...
Do not put literal credential values in YAML. Confirm that both referenced variables resolve in the selected Hermes profile before connecting. After `get_account_context` identifies the gym, confirm its IANA zone, make `AIMHARDER_GYM_TIME_ZONES` available to Hermes, and add `AIMHARDER_GYM_TIME_ZONES: "${AIMHARDER_GYM_TIME_ZONES}"` under `env`. Add `AIMHARDER_DEFAULT_GYM` in the same way only if several gyms need a default. An unset reference can remain literal and cause a configuration error, so do not add optional mappings before defining them.
