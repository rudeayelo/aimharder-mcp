...
The public versions `aimharder-mcp@0.1.0`, `0.1.1`, `0.1.2`, and `0.2.0` passed exact-registry-artifact live MCP checks in [validation](validation.md). The first-release functional QA verdict and its limits are also recorded there. The [README](../README.md) carries the consumer version pin. This page describes the Changesets and GitHub Actions release path agreed in the [automation ADR](adr/2026-09-24-github-actions-npm-release-automation.md).

**Activation status:** Changesets and GitHub Actions published `0.2.0` on 2026-09-25 through the configured GitHub App and npm OIDC trusted publisher. Its exact public artifact passed the authenticated local read-only MCP check recorded in [validation](validation.md#automated-npm-release-020-2026-09-25). npm publishing access requires two-factor authentication and disallows bypass-2FA tokens; the trusted publisher remains active.

## One-time setup and current configuration
...
5. Keep npm publishing access set to **Require two-factor authentication and disallow bypass 2fa tokens**. This setting was applied after the first OIDC release passed the exact-version local MCP check. No npm publish token is stored in GitHub Actions; remove any obsolete publish token if one is found elsewhere. Trusted publishing remains available with this setting.
...
The script installs that version from npm in a clean temporary directory/cache and checks the server over MCP stdio: initialization, the expected tool set (eleven for `0.2.0`), read-only account/gym access, gym selection, sanitized errors, and empty stderr. The npm subprocess receives no AimHarder credentials. It does not execute booking writes. Record the source revision, exact version, registry URL/integrity, UTC time, runtime, result, and limitations in [validation](validation.md). Mark the release's read-only artifact path as live-verified only after this check passes.
...
- `0.1.2` was the first automated release. The [version PR #17](https://github.com/rudeayelo/aimharder-mcp/pull/17) passed CI; the [release workflow](https://github.com/rudeayelo/aimharder-mcp/actions/runs/36023101635) published through npm OIDC and created [tag and GitHub Release `v0.1.2`](https://github.com/rudeayelo/aimharder-mcp/releases/tag/v0.1.2). Its authenticated exact-registry MCP check passed in [validation](validation.md#automated-npm-release-012-2026-09-24).
- `0.2.0` adds manual booking creation and cancellation. The [version PR #29](https://github.com/rudeayelo/aimharder-mcp/pull/29) passed CI; the [release workflow](https://github.com/rudeayelo/aimharder-mcp/actions/runs/36136864386) published through npm OIDC and created [tag and GitHub Release `v0.2.0`](https://github.com/rudeayelo/aimharder-mcp/releases/tag/v0.2.0). Its exact public artifact passed the authenticated read-only MCP check in [validation](validation.md#automated-npm-release-020-2026-09-25); its write actions were not retested from npm.
