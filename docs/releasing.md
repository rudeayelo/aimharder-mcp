...
The public versions `aimharder-mcp@0.1.0`, `0.1.1`, and `0.1.2` passed exact-registry-artifact live MCP checks in [validation](validation.md). The first-release functional QA verdict and its limits are also recorded there. The [README](../README.md) carries the consumer version pin. This page describes the Changesets and GitHub Actions release path agreed in the [automation ADR](adr/2026-09-24-github-actions-npm-release-automation.md).

**Activation status:** Changesets and GitHub Actions published `0.1.2` on 2026-09-24 through the configured GitHub App and npm OIDC trusted publisher. Its exact public artifact passed the authenticated local MCP check recorded in [validation](validation.md#automated-npm-release-012-2026-09-24). npm publishing access now requires two-factor authentication and disallows bypass-2FA tokens; the trusted publisher remains active.
