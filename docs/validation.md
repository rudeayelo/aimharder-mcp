...
## Release gate audit in progress (#13 and #12, 2026-09-24)
...
| Archive and protocol | On Node 24.14.0 with pnpm 12.5.1, `pnpm typecheck`, `pnpm test tests/package.test.ts`, `pnpm test tests/consumer.test.ts`, `pnpm build`, `pnpm test:package` and the full `pnpm test` suite (272 tests across 10 files) passed. The isolated archive had 37 allowlisted files, an installed executable, six tools and empty server stderr. | This is a local archive; the public registry version has not been published or checked. |
...
**QA verdict: pending.** The user-provided Hermes response verifies future WOD content; its quoted schedule wording correctly avoids inventing a reservable session. It does not establish the required combined future-content/session/booking comparison or a complete supported-query audit on this release candidate. The `queryTraining` composition returns `bookingSummary.status: "unconfirmed"` when no matching reservation is visible. The required future-content run of the combined read-only harness has not been recorded. #13 remains open and blocks npm publication #12.
