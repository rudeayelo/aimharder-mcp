...
The reusable consuming-client `queryTraining` composes the existing four MCP tools, with `scripts/query-training.mjs` as a runnable official SDK stdio example. It resolves tomorrow after selecting the gym and confirming its zone; API-client/MCP separation and the existing read-only operation set remain intact. See [usage](tools.md#questions-that-combine-tools), [runnable examples](development.md#run-and-inspect-locally), and [the composition decision](adr/2026-09-21-api-client-separated-from-interfaces.md#consuming-client-composition-2026-09-22).
...
The README now serves end users, with a six-tool/question table, desktop-client guide index, compatibility and coverage notice, secure setup path, and an unchecked future-capability roadmap. Shared configuration and tool references plus ChatGPT desktop, Claude Desktop, Hermes and OpenClaw guides are the only `docs/` Markdown included in the npm archive. Research, ADRs, validation history, the development guide and release procedure remain public in GitHub rather than in the consumer archive. The [distribution ADR](adr/2026-09-22-npm-distribution-for-mvp.md#consumer-documentation-subset-2026-09-24) records the package-content decision.

On Node 24.14.0 and pnpm 12.5.1, `pnpm typecheck`, `pnpm build`, `pnpm test` (272 passing tests in 10 files) and `pnpm test:package` passed. The isolated archive check found 23 allowlisted files, installed with production dependencies outside the checkout, initialized over stdio, listed six tools, checked fixture-based account/gym selection and sanitized errors, and observed empty server stderr. A local Markdown link and heading-anchor check found no missing targets across the 29 README, glossary and `docs/` Markdown files. These checks did not publish or download `aimharder-mcp@0.1.0` from npm.

The ChatGPT desktop STDIO setup form was supplied as a screenshot by the user; no end-to-end package connection was recorded there. Claude Desktop and OpenClaw instructions follow their current client documentation but were not tested with this package. Hermes evidence above concerns a locally installed archive and retains its stated limits; it does not establish registry-version behavior. No new real AimHarder requests were needed for this documentation/package-content update.

## Consumer setup corrections and default-zone behavior (2026-09-24)

The user requested `npx` as the generic client command, a Node `>=24` engine range, and optional gym zone configuration with an assumed `Europe/Madrid` default. The server now reports `timeZoneStatus: "assumed"` for an unmapped gym and `"user-confirmed"` for a configured IANA zone. This default is a product assumption; no new AimHarder zone field or multi-gym observation established it. Existing live comparisons used a separately confirmed zone and do not validate the fallback against another real gym. The [default-zone ADR](adr/2026-09-24-default-gym-time-zone.md) records the changed contract.

On Node 24.14.0, `pnpm typecheck`, `pnpm build`, `pnpm test` (272 passing tests in 10 files), and `pnpm test:package` passed. The package check installed the 23-file archive outside the checkout, initialized the MCP server, discovered six tools, and observed empty server stderr with anonymized fixtures. On Node 26.9.0, the TypeScript check, the same 272-test fixture suite, and the isolated package check also passed. Node 26 emitted experimental `localStorage` warnings from test workers; the assertions passed. These checks establish Node 24 and 26 behavior for the tested source and fixtures, not future Node versions or live behavior under the assumed zone. The npm registry version is still unpublished.
