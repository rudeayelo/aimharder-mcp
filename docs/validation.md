...
The booking implementation and minor Changeset merged through [PR #28](https://github.com/rudeayelo/aimharder-mcp/pull/28) after its `verify` check passed. The [version job](https://github.com/rudeayelo/aimharder-mcp/actions/runs/36136516906) created [release PR #29](https://github.com/rudeayelo/aimharder-mcp/pull/29). That PR updated the version, changelog, consumer pins, and packaged documentation to describe eleven tools and their write confirmations; its `verify` check passed. The release PR merged at source revision `2fc1785aa0d71620b44c205888cfd80ad2b1bc2a`.

The [release workflow](https://github.com/rudeayelo/aimharder-mcp/actions/runs/36136864386) passed typecheck, 336 fixture tests, build, and the isolated package check, then reported successful npm OIDC publication and created [tag and GitHub Release `v0.2.0`](https://github.com/rudeayelo/aimharder-mcp/releases/tag/v0.2.0). The public registry returned version `0.2.0`, [its exact tarball](https://registry.npmjs.org/aimharder-mcp/-/aimharder-mcp-0.2.0.tgz), and integrity `sha512-7uR0BfnHWBYwtUQeh/pQCCOqkLdzdSFOoLcGvRZQ4iIFvPIO5qbsI9cxGhRZket8uD2sG//L94wRPz7GxX3oiw==`. The registry metadata initially returned 404, then listed the version while its fixed tarball URL still returned a cached 404; both became available after propagation. Those responses were not treated as a failed or verified package.

At 2026-09-25 12:54 UTC, `scripts/registry-check.mjs 0.2.0` installed the exact public version into a clean temporary directory/cache and passed on Node 24.14.0. The authenticated SDK stdio check passed initialization, listing all eleven tools, read-only account/gym discovery for one accessible gym, explicit selection, inaccessible-gym rejection, sanitized errors, and empty server stderr. Existing private environment injection supplied the local AimHarder credentials without recording values. No booking or cancellation write was made against the published artifact. This verifies the public package's read-only account path and tool availability for the tested account and gym; it does not retest write execution, credit effects, late cancellation, other gyms, or any client app with `0.2.0`.
...
## Open Box cycle preflight (2026-09-25, issue #27)

Before any real write, the checkout passed Node 24.14.0 typecheck, build, 62 targeted public MCP booking/cancellation tests, the full fixture suite (334 tests in 14 files), and an isolated local-package check (25 files, eleven tools, empty server stderr). Credentials were supplied by an existing private environment mount; no values or raw account responses were recorded. The authenticated MCP SDK read confirmed one accessible 9NBC gym with the account holder's configured `Europe/Madrid` zone, read Open Box sessions over 25–30 September, and found no Open Box entry in the current upcoming-booking view. That view has no verified future horizon, so its absence alone does not prove an empty booking state for any date.

Initial creation previews returned `unsupported` because the real daily rows omitted `hidden`; the fixture rows had supplied `hidden=0`. An independent authenticated read established the omission while preserving `enabled=1`, `bookState=null`, `cancelledId=null`, and `resadmin=0`. The official public schedule renderer consults `hidden` only when labeling a cancelled class and does not require it to render the booking button. The preparation parser and anonymized public MCP fixtures were amended to accept an absent `hidden` field while rejecting a present nonzero value. After the change, read-only previews for Open Box on 29 September 20:00–21:00 and 30 September 10:00–11:00 reported `ready` and `unbooked`. This establishes an offered action in the retrieved schedule, not the final account eligibility, actual credit balance, booking-write response, or completed booking.

An independent daily read found `enabled=1` for Open Box through at least 7 October, beyond the account holder's reported five-day booking window. Thus that flag does not verify the actual booking horizon. The account holder clarified that this window means five calendar dates, 25–29 September inclusive. The furthest offered Open Box in that interval was selected: **29 September 2026, 20:00–21:00 Europe/Madrid**, at Nou Barris Cross Training. At the fresh 25 September 13:20 local preflight, its daily row was `ready` and `unbooked`, and the current upcoming view had no matching entry. The published 9NBC credit-loss boundary is 29 September 18:30 local, leaving about four days and five hours at that preflight. Final eligibility and balance were unverified. At that stage, separate action-time confirmations and write validation were still pending; no booking or cancellation POST had been sent.

## Open Box cycle validation (2026-09-25, issue #27)

The account holder separately confirmed creation and cancellation of the selected **Nou Barris Cross Training Open Box, 29 September 2026, 20:00–21:00 Europe/Madrid**. Its published no-credit-loss cancellation boundary was 29 September 18:30 local, about 101 hours away at the action prompts. The possible use of one credit came from the account holder's experience, not a verified balance or transaction record. A private environment mount supplied credentials; neither values nor source IDs were recorded.

For creation, a new read-only MCP preview matched the approved gym, confirmed zone, exact class and wall times, and `unbooked` state. One `POST /api/book` sent only `id` and `day`; HTTP 200 returned `bookState=1` with the other observed top-level keys `clasesContratadas`, `hasPublicMemberships`, and `id`. The MCP execution reported `confirmed` after fresh reads. A separate `prepare_booking_cancellation` read showed that exact class `booked` and cancellable, and a separate upcoming-view read contained one matching `booked` entry. This independently confirmed creation; HTTP status alone was not used as proof.

For cancellation, another fresh preview showed the same booked reservation and the account holder gave a new confirmation. One `POST /api/cancelBook` sent only `id` and `late=0`; HTTP 200 returned `cancelState=1`. The original MCP execution reported `uncertain`, because its rule required the follow-up row to retain the reservation ID. Immediate independent MCP reads showed the exact row `unbooked` and no matching upcoming entry. A separate authenticated source read confirmed the same class row had `bookState=null`, no positive `idres` or `cancelledId`, and no matching `nextClasses` entry. The account holder also checked AimHarder's UI and reported no reservation. No second cancellation request or late attempt was sent. The available 30-row history view had no matching entry, but its horizon is unverified.

The account's current no-booking state for this class and both standard request shapes were independently observed. The checkout's reconciliation was amended to accept `cancelState=1` plus the same source session released without `idres` and no conflicting upcoming entry. This corrected `confirmed` branch is fixture-tested after the live write; it was **not** exercised by another real cancellation. Other response branches, other gyms, late credit loss, credit use, and restoration remain unverified. No credit refund or balance is claimed.

After the reconciliation change, Node 24.14.0 typecheck and build passed. The targeted cancellation suites passed 26 tests; the final `pnpm test` run passed **336 tests in 14 files**. `pnpm test:package` installed the local 25-file archive outside the checkout, initialized the MCP server, discovered eleven tools, checked account/gym selection and sanitized errors, and observed empty server stderr. Local Markdown links and `git diff --check` passed. This package check does not publish or retest a real write.

## Booking release preparation (2026-09-25)

The `0.2.0` minor changeset covers the account-scoped manual booking tools from issues #22–#27. Before the feature pull request, Node 24.14.0 passed typecheck, build, all **336 tests in 14 files**, and an isolated 25-file package check with eleven tools and empty server stderr. The same typecheck, build, tests, and package check passed on Node 26.10.0; that runtime emitted only experimental local-storage warnings during tests. Both package checks asserted that the MCP initialization version matches the installed package version. Changesets reported a minor release, and `git diff --check` passed.

An authenticated, read-only MCP check of the **local checkout archive** also passed on Node 24.14.0: one accessible gym, explicit selection, inaccessible-gym rejection, eleven tools, sanitized errors, and empty server stderr. Existing private environment injection supplied credentials without recording values. No booking or cancellation write was made during release preparation. These results do not verify the future public npm artifact or extend the single Open Box write cycle above.
