...
`aimharder-mcp` is public on npm. Versions `0.1.0` and `0.1.1` passed authenticated exact registry-artifact verification. Version `0.1.2` was published automatically through npm OIDC and is the current consumer pin; its authenticated local MCP check is pending. The registry and client results are recorded below, separately from the functional QA verdict.
