...
export const gymSchema = z.object({ id: gymIdSchema, name: z.string(), timeZone: z.string().nullable(), timeZoneStatus: z.enum(['unverified', 'user-confirmed']) });
...
export function validateActivityResponse(value: unknown, gym: z.infer<typeof gymSchema>, startDate: string, endDate: string) {
  ...
  if (data.gym.id !== gym.id || data.gym.timeZone !== gym.timeZone || data.gym.timeZoneStatus !== 'user-confirmed'
